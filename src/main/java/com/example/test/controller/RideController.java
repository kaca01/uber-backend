...
public class RideController {
    ...
    public Ride findDriversActiveRide(@PathVariable int id) {
        return service.findDriversActiveRide(id);
    }

    @GetMapping("/active/{passengerId}")
    public Ride findPassengersActiveRide(@PathVariable int id) {
    ...
    @PutMapping("/{id}")
    public boolean cancelExistingRide(@PathVariable int id) {
        return service.cancelExistingRide(id);  //PRIJE NEGO VOZAC DODJE NA DESTINACIJU (to passenger moze)
    }

    //panic
    @PutMapping("/{id}/panic")
    public Message setPanic(@RequestBody String reason, @PathVariable int id) {
        return service.setPanic(reason, id);
    }

    // prihvati voznju
    @PutMapping("/{id}/accept")
    public Ride acceptRide(@PathVariable int id) {
        return service.acceptRide(id);
    }

    // prihvati voznju
    @PutMapping("/{id}/end")
    public Ride endRide(@PathVariable int id) {
        return service.endRide(id);
    }

    @PutMapping("/{id}/cancel")
    public Ride cancelRide(@RequestBody String reason, @PathVariable int id) {
